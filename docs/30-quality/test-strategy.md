...
| server paper runner 변경 | Python runner syntax check, local-only status/control API, 15m public candle fetch, file-based status/log persistence, duplicate paper evaluation guard, no private Bitget/order path |
