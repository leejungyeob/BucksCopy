...
## 0051. Replace ETH Vacuum With Fresh Wick Reclaim Strategy

- Status: accepted
- Date: 2026-06-02
- Context:
  - 사용자는 기존 ETH 후보들은 폐기된 것으로 보고, 처음부터 다시 시작하는 방식으로 ETHUSDT 전략을 재탐색하길 원했습니다.
  - 기존 `ETH 15m Vacuum Pulse`는 최근 4년 `+64.78%`, 전체 구간 `+46.28%`로 플러스였지만 BTC active 전략 대비 성과가 낮고 실전 후보로 만족스럽지 않았습니다.
  - 새 ETH 후보는 기존 폐기 후보 결과를 재사용하지 않고, 시간·요일 필터 없이 closed 15m candle과 공식 runner exit model로 검증해야 합니다.
- Decision:
  - `ETH 15m Vacuum Pulse`를 runtime registry, active route, 전략 카드, 접근 예시, deterministic fixture test에서 삭제합니다.
  - `ETH 15m Wick Reclaim`을 ETHUSDT의 유일한 active route로 추가합니다.
  - 전략 조건은 prior high/low sweep 후 강한 종가 reclaim, RSI exhaustion, volume/ATR filter, `5x` leverage, `5%` risk 기준입니다.
  - 공식 runner 최근 4년 결과는 `$100 -> $296.94`, `+196.94%`, `96` trades, win rate `51.04%`, MDD `21.70%`, PF `1.41`입니다.
  - 공식 runner 전체 보유 history 결과는 `$100 -> $228.30`, `+128.30%`, `128` trades, win rate `47.66%`, MDD `24.95%`, PF `1.29`입니다.
- Consequences:
  - 웹 전략 목록과 서버 live closed-candle 평가에서 ETH는 `eth-15m-wick-reclaim`만 후보로 생성됩니다.
  - 저장된 사용자 전략 선택이나 web access profile에 기존 `eth-15m-vacuum-pulse`가 남아 있으면 현재 registry 필터에서 제외됩니다.
  - 새 ETH 전략은 전체 구간에서도 플러스지만 PF `1.29` 수준이라 BTC 전략보다 보수적으로 forward 검증해야 합니다.
