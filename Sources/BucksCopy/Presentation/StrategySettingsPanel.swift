...
enum RiskParameterInfo {
    ...
    var message: String {
        ...
        case .maximumRiskPerTrade:
            return "이 거래가 실패해서 손절이 나갔을 때, 계좌 전체에서 최대 몇 %까지 잃어도 되는지를 정하는 값입니다.\n\n예를 들어 레버리지 8x, 진입가에서 손절가까지의 거리가 4%라면, 시드를 전부 넣었을 때 계좌 손실위험은 4% x 8 = 32%입니다. 그런데 1회 최대 손실을 12%로 정해두면, 앱은 12 / 32 = 37.5%까지만 투입해서 손절 시 계좌 손실이 12% 안쪽에 머물도록 줄입니다."
        case .maximumPositionMargin:
            return "한 번 진입할 때 시드 중 실제 증거금으로 넣을 수 있는 최대 비율입니다.\n\n앱은 먼저 손절 기준으로 안전하게 넣을 수 있는 비율을 계산하고, 그 다음 이 최대 투입 한도를 한 번 더 적용합니다. 그래서 손실 제한상 37.5%까지 들어갈 수 있더라도 1회 최대 투입이 20%라면 실제 투입은 min(37.5%, 20%) = 20%가 됩니다.\n\n즉 최대 손실은 '얼마나 잃어도 되는가'이고, 최대 투입은 '애초에 얼마까지 넣을 것인가'입니다."
        }
    }
}
